...
public class ConversationTableHelper {
    private static final String SQL_CREATE_ENTRIES =
            "CREATE TABLE " + ChatContract.ConversationTable.TABLE_NAME + " (" +
                    ...
                    ChatContract.ConversationTable.COLUMN_NAME_TYPE + ChatDbHelper.TEXT_TYPE + ChatDbHelper.COMMA_SEP +
                    ChatContract.ConversationTable.COLUMN_NAME_PROFILE_THUMB + ChatDbHelper.TEXT_TYPE + ChatDbHelper.COMMA_SEP +
                    ChatContract.ConversationTable.COLUMN_NAME_PROFILE_PIC + ChatDbHelper.TEXT_TYPE + ChatDbHelper.COMMA_SEP +
                    ChatContract.ConversationTable.COLUMN_NAME_GROUP_LEFT + ChatDbHelper.INTEGER_TYPE + ChatDbHelper.COMMA_SEP +
    ...
    public static ContentValues newUpdateGroupContentValues(String nickName, String message, long timeMillis) {
        ContentValues values = new ContentValues();
        values.put(ChatContract.ConversationTable.COLUMN_NAME_LATEST_MESSAGE, message);
        values.put(ChatContract.ConversationTable.COLUMN_NAME_TIME, timeMillis);
    ...
    public static ContentValues newInsertContentValues(String name, String nickname, String message, long timeMillis, int unread, String type) {
        ContentValues values = new ContentValues();
        ...
        values.put(ChatContract.ConversationTable.COLUMN_NAME_TYPE, type);
        return values;
    }
}